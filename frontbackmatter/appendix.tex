...
\begin{figure}[htbp]
    \includegraphics[width=8cm]{ObliviousSelection.png}
    \centering
...
% $ \left(u_0,\ldots,u_{\ell} \right)\in \left\{0,1\right\}^{\ell} $ is an uniform random $\ell$-bit string.
% In line $2,3$, we set the bits in $ b_0,\ldots,b_{\ell-1}$ to one when the corresponding bits in $u_0,\ldots,u_{\ell} $ are the leading zero bits, and other bits in $ b_0,\ldots,b_{\ell-1}$ to zero.
% Recall that the geometric distribution (cf.~\autoref{def:GeometricDistribution}) counts the number of Bernoulli trials up to and including the first success. Therefore, we append bit $1$ and compute the Hamming Weight of string $1, b_0,\ldots,b_{\ell-1}$.
...
\begin{protocol}[tbh!]
    ...
    \fbox{
        \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Protocol, Input, Output,FOR,},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{Geometric}$}]{
            \textbf{Input: None} \pcskipln \\
            \textbf{Output: $\left\langle {y}\right\rangle^{B,UINT}$, where $y\sim Geo\left(p=0.5\right) $} \\
            \text{$\left(u_0,\ldots,u_{\ell-1}\right)  \gets \Uppi^{RandBits}\left(\ell\right) $} \\
            \text{$\left(\left\langle p_{0}\right\rangle^B,\ldots,\left\langle p_{\ell-1}\right\rangle^B\right) =\Uppi^{PreOr}\left(\left\langle u_0\right\rangle^B,\ldots, \left\langle u_{\ell-1}\right\rangle^B\right) $} \\
            \text{$\left(\left\langle b_{0}\right\rangle^B,\ldots,\left\langle b_{\ell-1}\right\rangle^B\right)  = \left(\lnot \left(\left\langle p_{0} \right\rangle ^B\right),\ldots,\lnot \left(\left\langle p_{\ell-1} \right\rangle ^B\right) \right) $} \\
            % \text{$\left(\left\langle z_0^{ind}\right\rangle^B,\dots,\left\langle z_{\ell-1}^{ind}\right\rangle^B\right) =\left(1,\left\langle d_{0}\right\rangle ^B ,\ldots ,\left\langle d_{\ell-2}\right\rangle ^B\right) $. }\\
            \text{$\left\langle {y}\right\rangle^{B,UINT} =\Uppi^{HW}\left(\left\langle b_{0}\right\rangle^B,\ldots,\left\langle b_{\ell-1}\right\rangle^B\right) $.}
        }}
    \caption{SMPC protocol for sampling from a geometric distribution $Geo\left(0.5\right) $.}
...
% \paragraph{SMCP Protocol for Bernoulli Sampling Algorithm}
...
%         }}
%     \caption{SMCP Protocol for sampling from a Bernoulli distribution $ Bern\left(p\right) $.}
