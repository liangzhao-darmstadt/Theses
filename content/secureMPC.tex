...
For the first challenge, we implement MPC protocols that support (signed) integer arithmetic, fixed-point arithmetic, floating-point arithmetic, and data type conversions in the MOTION frameowrk~\cite{braun2020motion}.
...
To generate a uniform random fixed-point in the range $\left[0,1\right)$ with $f$ fractional bits and $k-f$ integer bits, we first generate $f$ random bits as the fractional part of the random fixed-point and set the $\left(k-f\right) $-bit integer part with zero bits.
                  ...
                  The MPC protocol for the snapping mechanism is to unfold the mathematical operations and replaces them with the corresponding MPC protocols. We reformulate $M_{S}\left(f\left(D\right),\lambda,B\right)$ in secret sharing form as follows:
                  \begin{equation}
                  ...
                  We integrate the protocols and present the complete MPC protocol for the snapping mechanism.
                  ...
                  % $\Lambda=2^n$ and can be calculated from the publicly known $\lambda$ (cf.~\autoref{subsubsec:calLambda}) without MPC.
...
$\Uppi^{SymmBino}\left(n,p=0.5\right) $ samples a symmetric binomial random variable $x\sim SymmBino\left(n,p=0.5\right) $ based on $Algo^{SymmetricBinomial}$ (cf.~\autoref{algo:SymmetricBinomialLargeN}).
      ...
      \begin{protocol}[tbh!]
            ...
            \caption{MPC protocol for sampling symmetrical binomial random variable $i\sim SymmBino\left(n,p=0.5\right) $.}
      ...
      We integrate the protocols to construct the complete MPC protocol for the Integer-Scaling Gaussian mechanism $\Uppi^{IsGauss}$.
      ...
      \begin{protocol}[tbh!]
            ...
            \caption{MPC Protocol for sampling geometric random variable $x \sim Geo\left(1-e^{-\frac{n}{d}}\right) $.}
      ...
      \begin{protocol}[tbh!]
            ...
            \fbox{
                  \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Protocol, Input, Output,FOR,TO},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{DGauss}\left(\sigma\right) $ }]{
                        ...
                        \text{$\left\langle {Y}\right\rangle ^{B,INT}\gets \Uppi^{ObliviousSelection}\left(\left\langle {Y_0}\right\rangle ^{B,INT},\ldots,\left\langle {Y_{ITER-1}} \right\rangle ^{B,INT},\left\langle b_0\right\rangle ^B,\ldots,\left\langle b_{ITER-1}\right\rangle ^B\right) $}
                  }}
            \caption{MPC Protocol for sampling discrete Gaussian random variable $Y \sim DGauss\left(\mu=0,\sigma\right) $.}
