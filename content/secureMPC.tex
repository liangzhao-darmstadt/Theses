\chapter{MPC Protocols for Differentially Private Mechanisms}
...
We first give a general overview of the settings and description our MPC-DP procedure.
...
Generally, we faces two technical challenges.
The first challenge is to identify the most efficient MPC protocols for arithmetic operations.
The second challenge is to determine the most efficient sampling algorithms for differentially private mechanisms.
For the first challenge, we implemente MPC protocols that support (signed) integer arithemtic, fixed-point arithemtic, floating-point arithemtic, and type conversions in MOTION frameowrk.
For the second challenge, we construct the MPC protocols for all the introduced sampling algorithms and evaluate the performance in \autoref{cha:benchmark}.

\textbf{Setting.} In our MPC-DP procedure, we consider $m$ users, and $n$ computation parties and one reconstruction party. Each user $U_i$ has private data $D_i$ and wish to compute $f\left(D_1,\ldots, D_m\right) $ and perturb it with the help of computation parties. The reconstruction party is responsible for reconstruct the computation result in plaintext. We assumes that the communication channels between users and computation parties are secure and authenticated, and the communication channels between computation parties are pair-wise secure and authenticated. The users can go offline after sending their secret shared private data to the computation parties. Then, the computation parties execute the interactive MPC protocols.
...
For the computation parties, we assume that the adversaries are semi-honest (i.e., they follows the protocol specifications but try to infer information) and there is at least one computation party that is not corrupted (i.e., full-threshold security).
% Other MPC protocols (e.g., secure again malicious adversaries (ref.???)) could also be applied in our MPC-DP procedure but may be less efficient.
% \section{General Procedure for Combining MPC and DP}
% \label{sec:ProcedureMPCDP}

\begin{figure}[htbp]
      \includegraphics[width=\textwidth]{MPC-DP procedure.PNG}
      \centering
...
(iii) The computation parties send tis share of the perturbed result to the reconstruction party, that reconstruct the perturbed result.
...
% % To summarize, a well-designed MPC-DP procedure requires a more careful integration of MPC protocols and differentially private mechanisms. \CHANGED{To guarantee the both the accuracy and privacy, we define several requirements, that is based on the "ideal scenario" with a centralized data server, i.e., a trusted server that has access to all the parties' data $D$ locally computes $f\left( D\right) $ before adding an essential amount of noise to guarantee DP}:

% % \begin{enumerate}
% % 	\item The MPC-DP procedure should achieve the same privacy protection level as in the centralized data server scenario, and the amount of noise $r$ (in above example: $r=\sum_{j=1}^{n} r_j$) should \CHANGED{exactly match that of the centralized data server scenario}.
% % 	\item The DP guarantee of the MPC-DP procedure should not degenerate in the presence of corrupted parties.
% % \end{enumerate}

% % \textbf{MPC-DP Procedure} It can be used among $n$ parties, or in an outsourcing scenario, i.e., an arbitrary amount of parties secret share their private inputs to $N$ ($N\ll n$) non-colluding computing parties. Then, the computing parties execute the $N$-party MPC-DP protocol, where they compute $f$ and add noise shares to get the noisy secret-shared results. Finally, the noisy secret-shared results are sent back to the parties for reconstruction. We assume semi-honest adversaries that follow the protocol specifications but wish to infer additional information\cite[Chapter 2]{evans2017pragmatic}.
...
\begin{enumerate}
      ...
      \item $\left(\left\langle y_1\right\rangle^B,\ldots,\left\langle y_{\ell}\right\rangle^B \right)\gets\Uppi^{Binary2Unary}\left(\left\langle \boldsymbol{x}\right\rangle ^{B},\ell\right) $~\cite{aliasgari2012secure} converts an unsigned integer $x$ from binary to unary bitwise representation and outputs a $\ell$-bit string $\boldsymbol{y}=\left( y_{0} ,\ldots , y_{\ell-1} \right) $, where the $x$ least significant bits $ y_{0} ,\ldots , y_{x-1} $ are set to $1$ and the rest bits $ y_{x} ,\ldots , y_{\ell-1} $ are set to $0$.
            ...
            % \item For signed ($INT$) and unsigned integer ($UINT$) operations (e.g., comparision, addition, substraction), we use depth-optimized circuits generated with HyCC (cf.~\autoref{subsection:MOTIONFramework}).

            % \item For floating-point operations, we use the Boolean circuits of work~\cite{DSZ15}.
...
We consider MPC protocols for arithmetic operations that are based on Boolean sharing GMW (BGMW), Yao sharing with BMR and arithmetic sharing GMW (AGMW).
Further, in our MPC protocols, we use integer, fixed-point and floating-point arithmetic interchangeably to achieve the best MPC performance (regarding communication and computation cost).

\textbf{Binary Circuit-Based Signed/Unsigned Integer.}
MOTION framework (cf.~\autoref{subsection:MOTIONFramework}) supports most unsigned integer arithmetic operations (e.g., +, -, *, /, and <). We extend it to support further arithmetic operations (e.g., modulo reduction and conversion operations with fixed-point and floating-point) by generating depth-optimized binary circuits for BGMW based unsigned integer and size-optimized binary circuits for BMR based unsigned integer with HyCC and CBMC-GC.
Except the arithmetic operations supported by the unsigned integer, we also implemente signed integer arithmetic that support operations such as, absolute value and negation.

\textbf{Binary Circuit-Based Floating-point.}
We consider 64-bit floating-point arithmetic~\cite{IEEE754_2019}. For operations such as, +, -, *, /, <, square, square root, exponentiation and logarithm, we use the binary circuits from ABY~\cite{DSZ15}. For other operations such as, conversion operations with fixed-point and integers, ceil, floor, absolute value, round to nearest integer, we use CBMC-GC to generate depth-optimized and size-optimized binary circuits with the C code from Berkeley SoftFloat library~\cite{BerkelySoftFloat}.

\textbf{Binary Circuit-Based Fixed-point.}
The main issues with fixed-point arithemtic is the inherent precision loss during arithmetic operations. Therefore, we implement fixed-point arithmetic in two precision modes:
(i) 64-bit fixed-point with 16-bit fractional part,
(ii) 64-bit fixed-point with 33-bit fractional part.
We use HyCC to directly generate the depth-optimized and size-optimized circuits for fixed-point operations such as, +, -, *, / and <.
For operations such as, exp2, log2, we use polynomial approximation~\cite{hart1978computer,aly2019benchmarking} and generate corresponding binary circuits in CBMC-GC.
For operations such as sqrt, we use both polynomial approximation~\cite{hart1978computer,aly2019benchmarking} and Goldschmidt approximation~\cite{markstein2004software,aly2019benchmarking}.

\textbf{AGMW-Based Floating-point.}
We implemente the arithemtic sharing GMW based 64-bit floating-point protocols primary based on the work of Aliasgari et al.~\cite{aliasgari2012secure}.
...
The arithmetic operations in work~\cite{aliasgari2012secure} are performed over a prime field $\mathbb{F}_p$ (modulo $p$) , whereas the arithetic operations in MOTION are performed in the ring field (modulo $\mathbb{Z} _{2^{n}}$). One major difference between these two field is that there is no inverse element (i.e., $a^{-1}$ in a prime field) in a ring field $\mathbb{Z} _{2^{n}}$. Therefore, we use the protocols (e.g., logical shifting, truncation, trunaction by a secret value, eta.) from works~\cite{escudero2020improved,dalskov2020secure} to replace the inverse element operation in work~\cite{aliasgari2012secure}.

\textbf{AGMW-based Fixed-point.}
We implement the arithemtic sharing GMW based fixed-point protocols from Catrina and Saxena's work~\cite{catrina2010secure}, where a fixed-point number $x$ is represented as $x = \overline{x}\cdot  2^{-f}$, where $\overline{x}\in \mathbb{Z} _{\left\langle k\right\rangle }$, $ \mathbb{Z} _{\left\langle k\right\rangle }=\left\{x\in \mathbb{Z}  \,|\,-2^{k-1}+1\leq x \leq 2^{k-1}-1\right\} $, and $f$ is the length of fraction bits. We choose $\left(k,f\right)=\left(41,20\right)  $ as~\cite{SCALEMAMBAv114} recommended.


\textbf{Random Number Generation}
As discussed in (cf.~\autoref{cha:secureDPMechanisms}), the differentially private mechanisms relies heavily on the generation of random numbers.
Generally, four types of random numbers are used in our protocols: (i) random BGMW bits, random unsigned integer in a range, random fixed/floating-point in range $\left(0,1\right) $.
To generate a random Boolean bit-string of length $\ell$, each computation party $P_i$ locally generate a random $\ell$-bit string $\boldsymbol{r_i}^B$ and set $\boldsymbol{r_i}^B$ as a Boolean sharing with GMW protocol of a public unknown random bit string. Let $\Uppi^{RandBits}\left(\ell\right) $ denote the MPC protocol for generating random bits of length $\ell$.

The last three type of random numbers are genereated based on random Boolean bits with additional conversion.

To generate a random unsigned integer in range $\left(0,m\right) $, we use the Simple Modular Method~\cite{NISTRandomNumber2015} that first generate a random bit string $\left\langle \boldsymbol{r}\right\rangle^B $ of length $\ell$-bit ($128$ or $256$), and compute $ \left\langle \boldsymbol{r}\right\rangle^B \mod m$ by taking $\left\langle \boldsymbol{r}\right\rangle^B $ as a $\ell$-bit unsigned integer. We keep the security parameter $s=\ell-\log_2 m$ greater than 64 by limiting $m<2^{64}$.
To generate a uniform random fixed-point in range $\left[0,1\right)$ with $f$ fractional bits and $k-f$ integer bits, we first generate $f$ random bits as the fractional part of the random fixed-point, and set the $k-f$-bit integer part with zero bits.

                  To generate a uniform random floating point in range $\left(0,1\right) $, we follow the methods discussed in \autoref{algo:RandFloat1} and construct corresponding MPC protocol \autoref{prot:RandFLoat1}.

                  As $\Uppi^{RandFloat1}$ shows, we first generate shares of mantissa bits $\left\langle d_{1}\right\rangle^{B}, \ldots ,\left\langle d_{52}\right\rangle ^{B} $ with $\Uppi^{RandBits}\left(52\right) $, and generate a share of geometric random variable $\left\langle \boldsymbol{x}\right\rangle^{B,UINT} $ with $\Uppi^{Geometric}\left(0.5\right) $ to build the biased exponent $\left\langle \boldsymbol{e}\right\rangle^{B,UINT}$, where $e=1023-\left(x+1\right) $. Finally, the parties use the mantissa and exponent bits to build the floating-point number $U^{*}=\left(1.d_{1} \ldots d_{52}\right)_{2} \times 2^{e -1023} $.
                  ...
                  \begin{protocol}[tbh!]
                        ...
                        \fbox{
                              \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Protocol, Input, Output},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{RandFloat1}$}]{
                                    ...
                                    \textbf{Output: $\left\langle \boldsymbol{U^{*}}\right\rangle ^{B,FL}$, where $U^{*}=\left(1.d_{1} \ldots d_{52}\right)_{2} \times 2^{e -1023} $} \\
                                    \text{$\left(\left\langle d_{1}\right\rangle ^B ,\ldots ,\left\langle d_{52}\right\rangle^B\right) \gets \Uppi^{RandBits}\left(52\right) $} \\
                                    \text{$\left\langle \boldsymbol{x}\right\rangle ^{B,UINT} \gets \Uppi^{Geometric}\left(0.5\right)  $} \\
                                    \text{$\left\langle \boldsymbol{e} \right\rangle^{B,UINT}\gets 1023-\left(\left\langle \boldsymbol{x}\right\rangle^{B,UINT}+1\right)  $} \\
                  ...
                  The MPC protocols for snapping mechanism is to unfold the mathematical operations and replaces them by the corresponding MPC protocols. We reformulate $M_{S}\left(f\left(D\right),\lambda,B\right)$ in secret sharing form as follows:
                  \begin{equation}
                  ...
                  Operation $\left\lfloor \left\langle x\right\rangle \right\rceil_{\Lambda} $ rounds input $x$ to the nearest multiple of $\Lambda$ (a public known value).
                  ...
                  The MPC protocol of operation $\text{clamp}_{B}$ can be directly realized with floating-point arithmetic operation and it is denoted by $\Uppi^{ClampB}$.
                  ...
                  \begin{protocol}[tbh!]
                        ...
                        \fbox{
                              \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Protocol, Input, Output},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{ClampB}\left(\left\langle {x}\right\rangle^{B,FL},B\right) $}]{
                                    ...
                                    \text{$\left\langle cond_{\left\lvert x\right\rvert <B}\right\rangle^B \gets \Uppi^{FLLt}\left(\Uppi^{FLAbs}\left(\left\langle {x} \right\rangle^{B,FL}\right) ,B\right) $}\\
                        ...
                        \caption{MPC protocol for $\text{clamp}_B\left(x\right) $.}
                        \label{prot:ClampB}
                  \end{protocol}
                  \FloatBarrier

                  \textbf{MPC Protocols for Snapping Mechanism.}
                  We integrate the protocols together and present the complete MPC protocol for snapping mechanism.
                  ...
                  \begin{protocol}[tbh!]
                        ...
                        \text{$ \left\langle {Y_{LapNoise}}\right\rangle ^{B,FL}=\Uppi^{FLMul}\left(\lambda ,\Uppi^{FLLn}\left(\left\langle {U^{*}}\right\rangle ^{B,FL}\right)\right)  $.}\\
                        ...
                        \caption{MPC protocol for snapping mechanism.}
                  ...
                  % According to our MPC-DP procedure (cf.~\autoref{cha:ProcedureMPCDP}), the parties generate a noise share $\left\langle r\right\rangle$ and add it to $\left\langle f\left(D\right)\right\rangle$ to achieve differential privacy (cf.~\autoref{sec:differentialPrivacy}). 


                  % In our MPC protocols, we mainly operate on IEEE 754 floating-point (cf.~\autoref{subsubsec:floatingPoint}): $\left\langle \boldsymbol{x}\right\rangle^{B,FL} =\left(\left\langle S\right\rangle ,\left\langle d_1\right\rangle,\ldots,\left\langle d_{52}\right\rangle, \left\langle e_1\right\rangle,\ldots,\left\langle e_{11}\right\rangle\right) $, where $x = \left(-1\right)^S \left(1.d_{1} \hdots d_{52}\right)_2 \times 2^{\left(e_1 \hdots e_{11}\right)_2-1023} $.
                  ...
                  % % In line $1$, the parties expand the biased exponent to $e=\left(0,e_1,\ldots,e_{11}\right) $ because $y$ (line $4$) is in interval $\left[-1022,1023\right] $.
                  ...
                  %         }}
                  %     \caption{MPC Protocol for $\left\lfloor x\right\rceil _\Lambda $}
                  %     \label{prot:Round2Delta}
                  % \end{protocol}
                  % \FloatBarrier


                  % \subsection{MPC-DP Protocol for Snapping Mechanism}
                  % \autoref{prot:SnappingMechanism} integrates our MPC protocols for snapping mechanism in~\autoref{sec:snappingMPC} into MPC-DP procedure (cf.~\autoref{cha:ProcedureMPCDP}).
                  % Recall (cf.~\autoref{eq:snappingMPC})
                  % \[\left\langle M_{S}\left( f\left(D\right) ,\lambda,B\right)\right\rangle =\text{clamp}_{B}\left(\left\lfloor\text {clamp }_{B}\left(\left\langle f\left(D\right)\right\rangle \right) \oplus \left\langle S\right\rangle \otimes \lambda\otimes \text{LN}\left(\left\langle U^{*}\right\rangle \right) \right\rceil_{\Lambda}\right).\]
                  % \end{equation}

                  % $\Lambda=2^n$ and can be calculated from the public known $\lambda$ (cf.~\autoref{subsubsec:calLambda}) without MPC.
                  % In line $5$, we directly manipulate sign bit of $\left\langle \boldsymbol{Y_{LapNoise}}\right\rangle ^{B,FL} $.




                  % \section{MPC Protocols for Integer-scaling Mechanism}
                  % In this section, we construct MPC protocols for Integer-Scaling Laplace mechanism (cf.~\autoref{sec:IntegerScalingLaplaceMechanism}) and Integer-Scaling Gaussian mechanism (cf.~\autoref{subsec:ISGauss}).

                  \section{MPC Protocols for Integer-Scaling Laplace Mechanism}
                  \label{sec:MPCProtocolsforInteger-ScalingLaplaceMechanism}
                  Recall that Integer-Scaling Laplace mechanism $M_{ISLap}\left(f\left(D\right),r,\varepsilon,\Delta _r\right)=f_r\left(D\right) +ir$ (cf.~\autoref{sec:IntegerScalingLaplaceMechanism}) use a discrete Laplace random variable $i \sim DLap\left(t=\frac{\Delta_r}{r\varepsilon}\right) $ to simulate continuous Laplace random variable.
                  ...
                  We first provide the MPC protocols for both algorithms, and then, the MPC protocols for Integer-Scaling Laplace mechanism.
            ...
            $\Uppi^{GeoExpBinarySearch}\left(\lambda\right) $ samples a geometric random variable $x\sim Geo\left(p=1-e^{-\lambda}\right) $ based on $Algo^{GeoExpBinarySearch}$ (cf.~\autoref{algo:GeometricExpBinarySearch}).
                  ...
                  In line $1-7$, each party locally calculates $L_0$, $R_0$, $M_0$ and $Q_0$ in plaintext to save MPC computations.
      ...
      \begin{protocol}[tbh!]
            ...
            \text{\t\t$\left\langle {M_j}\right\rangle ^{B,UINT}\gets$ ~\autoref{eq:ISMiddlePoint}}\\
            ...
            \text{$\left\langle x\right\rangle ^{B,UINT}\gets \Uppi^{ObliviousSelection}\left(\left\langle {R_0}\right\rangle ^{B,UINT},\ldots, \left\langle {R_{ITER-1}}\right\rangle ^{B,UINT},\left\langle fg_0\right\rangle ^{B},\ldots,\left\langle fg_{ITER-1}\right\rangle ^{B}\right) $}
            }}
            \caption{MPC protocol for sampling from geometric distribution $x \sim Geo\left(p=1-e^{-\lambda}\right) $.}
      ...
      Next, we construct $\Uppi^{TwoSideGeometric}$ that converts geometric random variable $x\sim Geo\left(p=1-e^{-\lambda}\right)$ to a discrete Laplace random variable $i\sim DLap\left(\frac{1}{\lambda}\right) $ based on $Algo^{TwoSideGeo}$~\autoref{algo:TwoSideGeometric}.
      ...
      In line $5$, we extract the sign correct sign $s$ and number part $g$ based on flags $\left\langle fg_0\right\rangle ^B,\ldots ,\left\langle fg_{iter-1}\right\rangle ^B$.
      ...
      \begin{protocol}[tbh!]
            ...
            \fbox{
                  \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Algorithm, Input, Output, IF,TO,RETURN, FOR,TO, ELSE IF, ELSE, WHILE, AND},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{TwoSideGeometric}\left(t\right) $}]{
                        ...
                        \text{$\left\langle {g} \right\rangle^{B,UINT}\|\left\langle s \right\rangle^{B} \gets \Uppi^{ObliviousSelection}\left(\left\langle {g_0}\right\rangle^{B,UINT} \| \left\langle s_0 \right\rangle^{B},\ldots,\left\langle {g_{ITER-1}}\right\rangle^{B,UINT} \| \left\langle s_{ITER-1} \right\rangle^{B},\left\langle fg_0\right\rangle^B,\ldots,\left\langle fg_{ITER-1}\right\rangle^B\right) $}\\
            ...
            \caption{MPC Protocol for two-side geometric distribution $x\sim DLap\left(t\right) $.}
      ...
      Next, we integrate the protocols to construct the complete MPC protocol for Integer-Scaling Laplace mechanism.
      ...
      % In our MPC-DP general framework (cf.~\autoref{prot:MPC-DP}), it can be reformulated as:
      ...
      where $r$, $\varepsilon$, $\Delta _r$ are public known values. We assume that the parties have already computed $\left\langle {f_r\left(D\right)}\right\rangle$.
      ...
      \begin{protocol}[tbh!]
            ...
            \fbox{
                  \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Protocol, Input, Output},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{ISLap}\left(\left\langle {f_r\left(D\right)}\right\rangle ^{B,FL},r,\Delta_r,\varepsilon\right) $}]{
                        ...
                        \text{$\left\langle {i}\right\rangle ^{B,UINT}\gets \Uppi^{TwoSideGeometric}\left(\lambda=\frac{1}{t}\right) $.}\\
      ...
      Recall that Integer-Scaling Gaussian Mechanism $M_{ISGauss}\left(f\left(D\right),r,\Delta_r,\varepsilon,\sigma\right)=f_r\left(D\right) +ir$ (cf.~\autoref{subsec:ISGauss}) use a symmetric binomial random variable $i\sim SymmBino\left(n,p=0.5\right)$ to simulate a Gaussian random variable.

      We first provide the MPC protocols for $Algo^{SymmetricBinomial}$, and then, the MPC protocols for Integer-Scaling Gaussian mechanism.
...
$\Uppi^{SymmBino}\left(n,p=0.5\right) $ samples a symmetric binomial random variable $x\sim SymmBino\left(n,p=0.5\right) $ based on $Algo^{SymmetricBinomial}$ (cf.~\autoref{algo:SymmetricBinomialLargeN}).
      ...
      Given value of $\sqrt{n}$, each party first compute following public known parameters:
      \begin{equation}
            \begin{split}
      ...
      \begin{protocol}[tbh!]
            ...
            \fbox{
                  \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Algorithm, Input, Output, IF,TO,RETURN, FOR, ELSE IF, ELSE, WHILE, TRUE, FALSE},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{SymmBinomial}\left(\sqrt{n}\right) $}]{
                        ...
                        \text{\t\t$\left\langle {p_{Bernoulli}}\right\rangle ^{B, FL}\gets  \Uppi^{FL\_MUL}\left( \left\langle {\tilde{p}_j}\right\rangle ^{B, FL} , \Uppi^{FL\_MUL}\left(\Uppi^{UINT2FL}\left({\Uppi^{UINT\_Exp2}\left({\left\langle {s_j}\right\rangle ^{B,UINT}}\right) }\right) ,\frac{m}{4}\right) \right) $}\\
                        \text{\t\t$\left\langle c_j\right\rangle ^B \gets \Uppi^{Bernoulli}\left(\left\langle {p_{Bernoulli}}\right\rangle ^{B, FL} \right) $}\\
            ...
            \caption{MPC protocol for sampling symmetrical binomial distribution $i\sim SymmBino\left(n,p=0.5\right) $.}
            \label{prot:SymmBinomialLargeN}
      \end{protocol}
      \FloatBarrier


      \paragraph{MPC Protocol for Integer-Scaling Gaussian Mechanism}
      \label{para:MPCProtocolforIntegerScalingGaussianMechanism}
      We integrate the protocols to construct the complete MPC protocol for Integer-Scaling Gaussian mechanism $\Uppi^{IsGauss}$.
      ...
      % % In our MPC-DP general framework~\autoref{prot:MPC-DP}, it can be reformulated as:
      Integer-scaling Gaussian mechanism can be reformulated in secret sharing as:
      \begin{equation}
            \begin{split}
            ...
            \end{split}
      \end{equation}
      where $r$, $\Delta _r$, $\varepsilon$,$\delta$ are public known values. We assume that the parties have already computed $\left\langle {f_r\left(D\right)}\right\rangle$.
      ...
      \begin{protocol}[tbh!]
            ...
            \fbox{
                  \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Protocol, Input, Output},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{ISGauss}\left(\left\langle {f_r\left(D\right)}\right\rangle^{B,FL},r,\sqrt{n}\right) $}]{
                        \textbf{Input: $\left\langle {f_r\left(D\right)}\right\rangle^{B,FL}$ ,$\lambda$} \pcskipln \\
            ...
            \caption{MPC-DP protocol for SNG Laplacian Mechanism.}
            \label{prot:ISGauss}
      \end{protocol}
      \FloatBarrier
      ...
      Recall that the discrete Laplace mechanism $M_{DLap}\left(f\left(D\right),r,\varepsilon\right)=f\left(D\right) +Y$ (cf.~\autoref{sec:discreteLaplacemechanism}) use discrete Laplace random variable $Y\sim DLap\left(\right) $ to perturb $f\left(D\right)$.
$Y$ can be generate with $Algo^{GeoExp}$ and $Algo^{DLap}$.

      We first provide the MPC protocols for above sampling algorithms, and then, the MPC protocols for discrete Laplace mechanism.


      % We present the MPC protocols for discrete Laplace and Gaussian mechanism (cf.~\autoref{sec:discreteGaussianMechanism}) in this section. 

      % We first present the MPC protocols for sampling geometric random variable based on \autoref{algo:GeometricExp}.
      ...
      \begin{protocol}[tbh!]
            ...
            \fbox{
                  \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Protocol, Input, Output,FOR,TO},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{GeometricExp}\left(n,d\right) $ }]{
                        ...
                        \text{$\left\langle u\right\rangle^{B, UINT}\gets \Uppi^{ObliviousSelection}\left(\left\langle u_0\right\rangle^{B, UINT}, \ldots, \left\langle u_{ITER_1-1}\right\rangle^{B, UINT}, \left\langle b^1_j\right\rangle ^B, \ldots,\left\langle b^1_{ITER_1-1}\right\rangle ^B \right)  $}\\
            ...
            \caption{MPC Protocol for sampling geometric $x \sim Geo\left(1-e^{-\frac{n}{d}}\right) $.}
      ...
      \begin{protocol}[tbh!]
            ...
            \fbox{
                  \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Protocol, Input, Output,FOR,TO},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{DLap}\left(t=\frac{d}{n}\right) $ }]{
                        ...
                        \text{$\left\langle {m}\right\rangle^{B,UINT}\|\left\langle s\right\rangle ^B\gets \Uppi^{ObliviousSelection}\left(\left\langle {m_0}\right\rangle ^{B,UINT}\|\left\langle s_0\right\rangle ^B,\ldots,\left\langle {{m_{ITER-1}}}\right\rangle ^{B,UINT}\|\left\langle s_{ITER-1}\right\rangle ^B,\left\langle fg_0\right\rangle ^B,\ldots,\left\langle fg_{ITER-1}\right\rangle ^B\right) $}\\
            ...
            \caption{MPC Protocol for sampling discrete Laplace $x \sim DLap\left(t\right) $.}
      ...
      \begin{protocol}[tbh!]
            ...
            \fbox{
                  \pseudocode[space=none, syntaxhighlight=auto, addkeywords={Protocol, Input, Output,FOR,TO},linenumbering, skipfirstln, head=\textbf{Protocol: $\Uppi^{DGauss}\left(\sigma\right) $ }]{
                        ...
                        \textbf{Output: $\left\langle {Y}\right\rangle ^{B,UINT}$, $Y\sim DGau \left(\mu=0,\sigma\right) $} \\
                        \text{$t\gets\left\lfloor \sigma\right\rfloor +1$}\\
                        \text{FOR $j\gets 0$ TO $ITER-1$ }\\
                        \text{\t\t$\left\langle {Y_j}\right\rangle ^{B,INT} \gets \Uppi^{DiscreteLap}\left(t\right) $}\\
                        \text{\t\t$\left\langle b_j\right\rangle ^B \gets \Uppi^{BernoulliEXP}\left(\frac{\left(\Uppi^{UI2FP}\left(\left\lvert \left\langle { Y_j}\right\rangle ^{B,INT}\right\rvert \right)  -\frac{\sigma ^{2}}{t}\right)^2 }{2\sigma^2}\right) $}\\
                        \text{$\left\langle {Y}\right\rangle ^{B,INT}\gets \Uppi^{ObliviousSelection}\left(\left\langle {Y_0}\right\rangle ^{B,INT},\ldots,\left\langle \boldsymbol{Y_{ITER-1}} \right\rangle ^{B,INT},\left\langle b_0\right\rangle ^B,\ldots,\left\langle b_{ITER-1}\right\rangle ^B\right) $}
                  }}
            \caption{MPC Protocol for sampling discrete Gaussian $Y \sim DGauss\left(\mu=0,\sigma\right) $.}
      ...
      \begin{protocol}[tbh!]
            ...
            \caption{MPC-DP protocol for Discrete Gaussian Mechanism.}
      ...
      In this section, we explain why our MPC protocols can guarantee computional privacy and differential privacy, i.e., a semi-honest adversary learns nothing beyond the output of the protocol and the reconstructed output satisfy differential privacy. Generally, the computional privacy guarantee of our MPC protocols follows directly from the Boolean GMW sharing (ref.). At the beginning of the MPC-DP protocol, each user secret share their input to $ N$ non-colluding computation parties. For computation parties, these secret-shared values are indistinguishable random values and leak nothing about the users' private input. Then, the computation parties collaboratively generate different types of noise that is based on generating random Boolean GMW bit strings $\left\langle \boldsymbol{r_0}\right\rangle^B, \ldots, \left\langle \boldsymbol{r_{N-1}}\right\rangle^B, $. As $\left\langle \boldsymbol{r_0}\right\rangle^B, \ldots, \left\langle \boldsymbol{r_{N-1}}\right\rangle^B$ are generated by each computation party locally and $\left\langle \boldsymbol{r_0}\right\rangle^B \oplus \left\langle \boldsymbol{r_1}\right\rangle^B \oplus  \ldots \oplus  \left\langle \boldsymbol{r_{N-1}}\right\rangle^B$ is public unknown to all computing parties, the random noise after running MPC protocols with random bis strings guarantee the computional privacy. Differential privacy is directly satisfied because of computional privacy.
