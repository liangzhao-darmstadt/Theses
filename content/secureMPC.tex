...
To generate a uniform random fixed-point in the range $\left[0,1\right)$ with $f$ fractional bits and $k-f$ integer bits, we first generate $f$ random bits as the fractional part of the random fixed-point and set the $\left(k-f\right) $-bit integer part with zero bits.
                  ...
                  \textbf{\smpc Protocols for Snapping Mechanism.}
                  We integrate the protocols and present the complete \smpc protocol for the snapping mechanism.
                  ...
                  \begin{protocol}[tbh!]
                        ...
                        \caption{\smpc protocol for snapping mechanism.}
            ...
            $\Uppi^{GeoExpBinarySearch}\left(\lambda\right) $ samples a geometric random variable $x\sim Geo\left(p=1-e^{-\lambda}\right) $ based on $Algo^{GeoExpBinarySearch}$ (cf.~\autoref{algo:GeometricExpBinarySearch}).
                  ...
                  In line $1-7$, each party locally calculates $L_0$, $R_0$, $M_0$ and $Q_0$ in plaintext to save \smpc computations.
...
$\Uppi^{SymmBino}\left(n,p=0.5\right) $ samples a symmetric binomial random variable $x\sim SymmBino\left(n,p=0.5\right) $ based on $Algo^{SymmetricBinomial}$ (cf.~\autoref{algo:SymmetricBinomialLargeN}).
      ...
      \begin{protocol}[tbh!]
            ...
            \caption{\smpc protocol for sampling symmetrical binomial random variable $i\sim SymmBino\left(n,p=0.5\right) $.}
            \label{prot:SymmBinomialLargeN}
      \end{protocol}
      \FloatBarrier


      \paragraph{SMPC Protocol for Integer-Scaling Gaussian Mechanism}
      \label{para:MPCProtocolforIntegerScalingGaussianMechanism}
      We integrate the protocols to construct the complete \smpc protocol for the Integer-Scaling Gaussian mechanism $\Uppi^{IsGauss}$.
      ...
      \begin{protocol}[tbh!]
            ...
            \caption{SMPC Protocol for sampling geometric random variable $x \sim Geo\left(1-e^{-\frac{n}{d}}\right) $.}
      ...
      \begin{protocol}[tbh!]
            ...
            \caption{SMPC Protocol for sampling discrete Gaussian random variable $Y \sim DGauss\left(\mu=0,\sigma\right) $.}
